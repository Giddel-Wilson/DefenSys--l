...
{:else}
	<div
		class="flex flex-col md:flex-row h-screen overflow-hidden bg-gradient-to-br from-slate-50 to-blue-50"
	>
		...
		<!-- Sidebar: Scans List -->
		<div
			class="flex w-full md:w-96 flex-col border-r border-slate-200 bg-white {isLoaded
		...
		>
			...
			<!-- Filters -->
			<div class="border-b border-slate-200 p-6 space-y-4">
				<!-- Search -->
				<div>
					<input
						type="text"
		...
		<div class="flex flex-1 flex-col overflow-hidden">
			<!-- Header -->
			<div class="hidden md:block border-b border-slate-200 bg-white px-4 md:px-8 py-4 md:py-6">
				<div class="flex flex-col md:flex-row items-start md:items-center justify-between space-y-3 md:space-y-0 {isLoaded ? 'animate-[fadeInDown_0.6s_ease-out]' : 'opacity-0'}">
					<div>
					...
					<div class="flex items-center space-x-2 md:space-x-4">
						<button class="hidden md:block rounded-lg p-2 transition-colors hover:bg-slate-100" aria-label="Search">
							<svg
						...
						<div class="flex items-center space-x-2 md:space-x-3 border-l border-slate-300 pl-2 md:pl-4">
							...
							<button
								on:click={() => goto('/dashboard')}
								class="rounded-lg bg-gradient-to-r from-cyan-500 to-blue-500 px-3 md:px-4 py-2 text-xs md:text-sm font-medium text-white transition-all duration-300 hover:scale-105"
							>
								Dashboard
							</button>
						</div>
			...
			{#if !selectedScan}
				<!-- Empty State -->
				<div
					class="flex flex-1 items-center justify-center p-4 {isLoaded
			...
			{:else}
				...
				<div class="flex flex-1 flex-col overflow-hidden">
					...
					<div class="border-b border-slate-200 bg-white p-4 md:p-6">
						<div class="flex flex-col md:flex-row items-start justify-between space-y-3 md:space-y-0">
							<div class="flex-1">
							...
							<button
								...
								class="flex items-center space-x-2 rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 px-4 md:px-6 py-2 md:py-3 text-sm md:text-base font-semibold text-slate-900 shadow-lg shadow-cyan-500/20 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-cyan-500/30"
							>
					...
					<div class="flex-1 overflow-y-auto p-4 md:p-6">
						<div
							class="rounded-xl md:rounded-2xl border border-slate-200 bg-white shadow-xl {isLoaded
									...
									{#each vulnerabilities as vuln, i}
										...
										>
											<div class="flex items-start justify-between">
												<div class="flex-1">
													<div class="mb-2 flex items-center space-x-2 md:space-x-3">
														<span
															class="inline-block rounded-full bg-gradient-to-r {getSeverityColor(
																vuln.severity
															)} px-2 md:px-3 py-0.5 md:py-1 text-[10px] md:text-xs font-bold text-slate-900 shadow-lg"
														>
															{vuln.severity}
														</span>
														<span class="text-[10px] md:text-xs text-slate-900/50">{vuln.type}</span>
													</div>
													...
													{#if vuln.affectedUrl}
														<div class="mt-3 flex items-center space-x-2 text-xs text-slate-900/50">
															...
															<span class="line-clamp-1">{vuln.affectedUrl}</span>
														</div>
													{/if}
													...
													{#if selectedVulnerability?.id === vuln.id}
														<div class="mt-4 space-y-4 rounded-xl bg-slate-50 p-4">
															{#if vuln.evidence}
																<div>
																	<h4 class="mb-2 text-sm font-semibold text-slate-900/90">
																		Evidence
																	</h4>
																	<pre
																		class="overflow-x-auto rounded-lg bg-black/40 p-3 text-xs text-slate-900/70">{vuln.evidence}</pre>
																</div>
															{/if}
															...
															{#if vuln.recommendation}
																<div>
																	...
																	<p class="text-sm text-slate-900/70">{vuln.recommendation}</p>
																</div>
															{/if}

															{#if vuln.cwe}
																<div>
																	<h4 class="mb-2 text-sm font-semibold text-slate-900/90">CWE</h4>
																	<p class="text-sm text-slate-900/70">{vuln.cwe}</p>
																</div>
															{/if}
														</div>
													{/if}
												...
												<svg
													class="ml-4 h-5 w-5 text-slate-900/50 transition-transform {selectedVulnerability?.id ===
													vuln.id
														? 'rotate-180'
														: ''}"
													fill="none"
